...
- **[bookcost/core/](bookcost/core/)** — pure logic:
  - [calculator.py](bookcost/core/calculator.py) — `CostCalculator`: cost math plus domain constants (`OPTIMAL_SPECS` per trim-size/قطع, `ZINC_DIMS`, `BOOK_PAGE_DIMS`, `COST_GROUPS` — the cost fields in 4 sections, `BOOK_TYPE_PRESETS` — visible fields per book type, `None` = show all). Paper helpers: `zinc_size_label`, `actual_print_size` (cut-in-half), `sheets_needed`/`bought_paper_count`.
  - [cost_model.py](bookcost/core/cost_model.py) — **unified cost engine**: `CalcType` (fixed/per_tiraj/per_form/per_volume), `CostLine`, `CostContext(tiraj, total_forms, volume_count)`, `resolve_total`/`project_total`/`resolved_breakdown`. Every cost — built-in, custom, or a sub-line — is a `CostLine`; the grand total resolves each amount by its calc type. This (not a plain sum) is how totals are computed.
  ...
  - [project_io.py](bookcost/core/project_io.py) — versioned export/import of single projects as `.ketab` files (UTF-8 JSON inside; unknown columns ignored, missing default to NULL; the installer registers the file association and `main.py` imports a path passed as argv). DB backup/restore helpers live on `BookDatabase` (`backup_to`, `is_valid_database_file`), wired to the فایل menu.
  - Domain features carried across the stack, each with its own table + widget: **multi-volume** projects (one project owns all volumes — `project_volumes` rows of name/pages/forms + `VolumesWidget`; the details tab sums them to drive the shared paper/zinc machinery and sets `volume_count`); **unified cost lines** (`project_cost_lines` — per-field calc type, custom named lines, and subfields via `parent_key` + `CustomCostWidget`); **multiple paper types** per section (`project_papers` + `PaperListWidget`); translation as a percentage (`tarjomeh_percent`). Built-in cost amounts still mirror to `hazineh_*` columns; calc types + customs live only in `project_cost_lines`.
...
Data flow: `DetailsTab` collects inputs → `DetailsTab.build_cost_lines()` → `cost_model.project_total` (resolved against `CostContext`) → `BookDatabase` persists (`projects` + `project_details` + `project_volumes`/`project_papers`/`project_cost_lines`) → `reporting` renders the PDF (rows resolved so they sum to the total).
...
- `style.qss` holds the **light** theme (white surfaces, slate text) and pins the app font (Tahoma 14px). Inline widget colors must be the semantic set documented at the top of `style.qss` (hint `#1d4ed8`, success `#15803d`, error `#b91c1c`, warning `#b45309`) — all ≥4.5:1 on white; never reuse dark-theme colors like `#4caf50`/`#ffb74d`. Auto-computed cost fields are tinted via the `autoField` dynamic property, not inline styles.
