...
- **[bookcost/core/](bookcost/core/)** — pure logic:
  ...
  - Domain features carried across the stack: multi-volume series (`projects.series_name/volume_no/series_volumes` — cover paper/zinc auto-costs and the whole-series هزینه چاپ جلد are split across volumes via `apply_series_adjustments`), multiple paper types per section (`project_papers` table + `PaperListWidget`; entries replace the single form/price inputs), and translation as a percentage (`tarjomeh_percent`, applied in `compute_totals` beside royalty).
